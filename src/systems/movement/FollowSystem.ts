...
export class FollowSystem extends System {
    ...
    public execute(delta: number, time: number): void {
        ...
        for(const entity of this.queries.entities.results) {
            if (entity.getComponent(FollowComponent)!.follow != null && entity.getComponent(MovePathComponent)?.path != null) {
    ...
    private updateFollowingEntity(entity: Entity): void {
        ...
        movePathComponent.path = convertPathfindingPathToPositions(path.slice(1).slice(0, -1));// move to the other entity
    }
}