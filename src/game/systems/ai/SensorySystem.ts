...
const processAliveEvent = (
  ...
  if (isSameEntity(entity)(event.entity)) return;

  if (!isInRange(entity, sensory.sensor.range)(event.entity)) return;

  const range = distance(entity, event.entity);

  sensory.sensor.addModality(new Modality(event.entity, range));

  // process event.entity too
  event.entity
    .getComponent(Sensory)
    ?.sensor.addModality(new Modality(entity, range));
};
...
export const SensorySystem = createSystem({
  ...
  .withRunFunction(
    ({ entityAdded, entityRemoved, died, idled, moved, collided, query }) => {
      const entities = Array.from(query.iter());

      const aliveEvents: EntityEvent[] = [entityAdded, moved, idled, collided]
