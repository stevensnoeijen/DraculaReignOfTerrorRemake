...
type MouseEventListenerOrEventListenerObject = MouseEventListener | MouseEventListenerObject;
...
export class MouseEventAdapter {
    ...
    public addEventListener(
        type: EventTypes,
        listener: MouseEventListenerOrEventListenerObject | null,
        options?: boolean | AddEventListenerOptions
    ): void {
        // cast as normal event-listener for the eventTarget
        this.eventTarget.addEventListener(type, listener as EventListenerOrEventListenerObject, options);
    }

    public removeEventListener(
        type: EventTypes,
        callback: MouseEventListenerOrEventListenerObject | null,
    ...
    public handleMouseMove(event: MouseEvent): void {
        if (1 === event.buttons) {
            if (
                !this.dragged &&
                event.movementX + event.movementY < MouseEventAdapter.DRAG_THRESHOLD
            ) {
            ...
            if (!this.dragged) {
                ...
                this.eventTarget.dispatchEvent(
                    new CustomMouseEvent('leftmousedragstart', event)
                );
                return;
            }

            this.eventTarget.dispatchEvent(
                new CustomMouseEvent('leftmousedragmove', event)
            );
        }
    }
    ...
    public handleMouseUp(event: MouseEvent): void {
        if (MouseEventAdapter.MOUSE_BUTTON_LEFT === event.button) {
            if (
                ...
                Date.now() - this.lastClickedTimeStamp <=
                MouseEventAdapter.DOUBLECLICK_TIMEOUT
            ) {
                if (this.singleClickTimeout) {
                    this.cancelSingleClick();
                }
                this.eventTarget.dispatchEvent(
                    new CustomMouseEvent('leftmousedoubleclick', event)
                );
            } else {
                if (!this.dragged) {
                    this.singleClickTimeout = setTimeout(
                        this.createSingleClickHandler(
                            new CustomMouseEvent('leftmouseclick', event)
                        ),
                        MouseEventAdapter.DOUBLECLICK_TIMEOUT
                    );
                }
            ...
            // right button
            this.eventTarget.dispatchEvent(
                new CustomMouseEvent('rightmouseclick', event)
            );
        }
        ...
        if (this.dragged) {
            this.dragged = false;
            this.eventTarget.dispatchEvent(
                new CustomMouseEvent('leftmousedragend', event)
            );
        }
    }
